...
function loadLand()
{
    if (isOkToCall)    //we could update only if accounts accessible via metamask..
    {
        ...
        realEstateContractHook.getNoOfLands.call(ethWeb3.eth.defaultAccount,
            ...
            {
                console.log("No of Properties owned by Current Account: " + noOfLandsOfCurrentAccount);
                for (index=0; index<noOfLandsOfCurrentAccount;index++)
                {
                    realEstateContractHook.getLand.call(ethWeb3.eth.defaultAccount, index, 
                        (error, land) =>
                        {
                            if (land[3] != 0)
                            {
                                console.log("Property ID: " + land[3] + ":");
                                console.log("Location: " + land[0]);
                                console.log("Price: " + land[1]);
...
function updateListDisplay(land)
{
    ...
    var myCard = $('<div class="card" id="inlineCard' + i + '" style="width:250px"><img src="https://s14.postimg.org/b7n0s9csh/land.jpg"><h5>Property ID: '+ i +'</h5><span>Location: '+ location +'</span><span>Price: '+ cost +'</span></div>');
